search.beta0<-function(beta,Z1,Z2,n,q,fix.seed=1){
  dx<-as.numeric(cbind(1,Z1,Z2)%*%beta>=0)
  eza1<-exp(-Z1+dx*(Z1-Z2))   # exponential of linear predictors in PH model of risk 1
  eza2<-exp(-dx*(2+Z1+2*Z2))  # rate of expoential dist. for risk2
  p1<-1-(1-q)^eza1
...
return(list(T=T,ebs=ebs))
}

search.beta1<-function(beta,Z1,Z2,n,q,t0=2,fix.seed=1){
  sb<-search.beta0(beta,Z1,Z2,n,q,fix.seed)
  return(mean(as.numeric(sb$T<=t0 & sb$ebs==1)))
}

search.beta2<-function(beta,Z1,Z2,n,q,t0=2,fix.seed=1){
  sb<-search.beta0(beta,Z1,Z2,n,q,fix.seed)
  return(mean(as.numeric(sb$T<=t0 & sb$ebs==2)))
}

# optim function
search.beta<-function(beta,Z1,Z2,n,q,t0=2,fix.seed=1){
  sb<-search.beta0(beta,Z1,Z2,n,q,fix.seed)
  F1.b<-mean(as.numeric(sb$T<=t0 & sb$ebs==1))
  F2.b<-mean(as.numeric(sb$T<=t0 & sb$ebs==2))
  return(c(F1.b,F2.b))
}
